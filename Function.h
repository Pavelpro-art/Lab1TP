...
void menuOfEdition(T& obj, string s) { //передаем строчку, которая содержит в себе имя файла (ориентир на нужный файл)
	...
	while (flag) {
		...
		system("cls");
		switch (button) {
		case 1: obj.displayData(); cout << "На какое место вставить новый элемент: "; cin >> id; obj.addElement(id); system("pause"); break;
		case 2: obj.displayData(); cout << "Какой элемент удалить: "; cin >> id; obj.deleteElement(id); system("pause"); break;
		case 3: obj.displayData(); system("pause"); break;
		case 4: obj.displayData(); cout << "Какой элемент изменить: "; cin >> id; obj.dataChange(id); system("pause"); break;
